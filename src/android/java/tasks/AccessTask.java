...
import id.nocola.cordova.plugin.cs108.CustomMediaPlayer;
...
public class AccessTask extends AsyncTask<Void, String, String> {
    ...
    boolean button;
    String buttonText;
    String registerRunTime, registerTagGot, registerVoltageLevel;
    String registerYield, registerTotal;
    ...
    private static CallbackContext currentCallback;

    public AccessTask(boolean button, String textViewWriteCount, boolean invalidRequest,
                      ...
                      String registerRunTime, String registerTagGot, String registerVoltageLevel,
                      String registerYieldView, String registerTotalView, CallbackContext callbackContext) {
        ...
        MainActivity.mCs108Library4a.appendToLog("invalidRequest = " + invalidRequest);
    ...
    protected void onPreExecute() {
        ...
        if (invalidRequest == false) {
            if (strPassword.length() != 8) {
                invalidRequest = true;
                MainActivity.mCs108Library4a.appendToLog("strPassword.length = " + strPassword.length() + " (not 8).");
        ...
        if (invalidRequest == false) {
            if (DEBUG)
                MainActivity.mCs108Library4a.appendToLog("AccessTask(): powerLevel = " + powerLevel);
            int matchRep = 0;
            ...
            if (powerLevel < 0 || powerLevel > 330) invalidRequest = true;
                ...
                invalidRequest = true;
                MainActivity.mCs108Library4a.appendToLog("setSelectedTag is failed with selectMask = " + selectMask + ", selectBank = " + selectBank + ", selectOffset = " + selectOffset + ", powerLevel = " + powerLevel);
            }
        }
        gotInventory = false;
...
//                    if (registerTagGot != null) registerTagGot.setText(output[1]);
                    ...
                    if (timePeriod > 0) {
//                        if (registerRunTime != null)
//                            registerRunTime.setText(String.format("Run time: %d sec", timePeriod));
                    }
                } else if (taskCancelReason == TaskCancelRReason.NULL) {
//                    if (registerVoltageLevel != null)
//                        registerVoltageLevel.setText(MainActivity.mCs108Library4a.getBatteryDisplay(true));
                }
            } else {
...
//            if (registerTotal != null) registerTotal.setText("Total:" + Integer.toString(++total));
    ...
    protected void onCancelled() {
        super.onCancelled();
        if (DEBUG)
            MainActivity.mCs108Library4a.appendToLog("AccessSecurityLockFragment.InventoryRfidTask.onCancelled()");
    ...
    protected void onPostExecute(String result) {
        if (DEBUG)
            MainActivity.mCs108Library4a.appendToLog("AccessSecurityLockFragment.InventoryRfidTask.onPostExecute(): " + result);
    ...
    void DeviceConnectTask4RegisterEnding() {
        ...
        if (false) {
            ...
            if ((repeat == false && taskCancelReason != TaskCancelRReason.NULL) || backscatterError != 0 || accessError != 0 || accessResult == null || resultError.length() != 0) {
                MainActivity.mCs108Library4a.appendToLog("FAILURE");
                currentCallback.error("FAILURE");
                playerO.start();
            } else {
                MainActivity.mCs108Library4a.appendToLog("SUCCESS");
                currentCallback.success("SUCCESS");
                playerN.start();
            }
        } else {
            strErrorMessage = "";
            switch (taskCancelReason) {
                case NULL:
                    if (accessResult == null || (resultError != null && resultError.length() != 0) || (endingMessaage != null && endingMessaage.length() != 0))
                        strErrorMessage += ("Finish as COMMAND END is received " + (gotInventory ? "WITH" : "WITHOUT") + " tag response");
                    else
                        currentCallback.success("SUCCESS");
                    break;
                case STOP:
        ...
        if (endingMessaage != null) if (endingMessaage.length() != 0)
            strErrorMessage += "Received CommandEND Error = " + endingMessaage;
        if (strErrorMessage.length() != 0) endingMessaage = strErrorMessage;
//        button.setText(buttonText);
        if (endingMessaage != null) {
            if (endingMessaage.length() != 0) {
                currentCallback.success(endingMessaage);
            }
        }
    }
}
