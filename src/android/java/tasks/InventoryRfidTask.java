...
//            Toast.makeText(MainActivity.mContext, "Invalid Request.", Toast.LENGTH_SHORT).show();
            this.mCallbackContext.error("Invalid Request.");
        }
...
//                    MainActivity.mCs108Library4a.appendToLogView("CANCELLING. sent abortOperation");
                ...
                discoverCallback.error("Finishing tag data upload");
            } else if (DEBUG)
            ...
            long currentTime = 0;
            {
                while (rx000pkgDataArrary.size() != 0) {
                    ...
                    String scanResult = new Gson().toJson(tagsList);
//                    Log.e("scanResult=", scanResult);
                    ...
                    if (tagsList == null) {
                        ...
                        if (iMatchItem >= 0) {
                            ...
                            Log.e("jsone=", json);
                            PluginResult result1 = new PluginResult(PluginResult.Status.OK, json);
                            result1.setKeepCallback(true);
                            mCallbackContext.sendPluginResult(result1);
                        }
                    }
...
//            if (false) MainActivity.mCs108Library4a.appendToLogView("Playing old song");
    ...
    protected void onPostExecute(String result) {
        ...
        MainActivity.mCs108Library4a.appendToLog("result=" + result);
        DeviceConnectTask4InventoryEnding(taskCancelReason);
    }
    ...
    public InventoryRfidTask(Context context, int extra1Bank, int extra2Bank, int data1_count, int data2_count, int data1_offset, int data2_offset,
                             ...
                             ArrayList<ReaderDevice> tagsList, boolean selectDupElim, TextView geigerTagRssiView,
                             String strMdid,
                             TextView rfidRunTime, TextView geigerTagGotView, TextView rfidVoltageLevel,
                             TextView rfidYieldView, Button button, TextView rfidRateView, CallbackContext callbackContext) {
...
//        this.readerListAdapter = readerListAdapter;
        this.needDupElim = selectDupElim;
...
//            MainActivity.mCs108Library4a.appendToLogView("playerO and playerN is created");
    ...
    void DeviceConnectTask4InventoryEnding(TaskCancelRReason taskCancelRReason) {
        ...
        MainActivity.mCs108Library4a.appendToLog("1INVENDING: Ending with endingRequest = " + endingRequest);
        if (MainActivity.mContext == null) return;
//        if (readerListAdapter != null) readerListAdapter.notifyDataSetChanged();
        if (endingRequest) {
            switch (taskCancelReason) {
                case NULL:
                    discoverCallback.error("Finish as COMMAND END is received");
                    break;
                case STOP:
                    discoverCallback.error("Finish as STOP is pressed");
                    break;
                case BUTTON_RELEASE:
                    discoverCallback.error("Finish as BUTTON is released");
                    break;
                case TIMEOUT:
                    discoverCallback.error("Finish as TIMEOUT");
                    break;
                case RFID_RESET:
                    discoverCallback.error("Finish as RFID RESET");
                    break;
                case INVALD_REQUEST:
                    discoverCallback.error("Invalid sendHostRequest. Operation is cancelled.");
                    break;
            }
