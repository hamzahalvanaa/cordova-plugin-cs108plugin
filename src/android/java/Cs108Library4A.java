...
//                if (retValue)
    ...
    */
    ...
    @Keep
    Object getDeviceStatus(boolean settingDisplay) {
        float floatValue = (float) getBatteryLevel() / 1000;
        if (floatValue == 0) return " ";
        if ((settingDisplay && getBatteryDisplaySetting() == 0))
            return String.format("%.3f V", floatValue);
        else if (true) {
            JSONObject deviceStatus = new JSONObject();
            try {
                deviceStatus.put("battery", (getBatteryValue2Percent(floatValue) + "%"));
                deviceStatus.put("voltage", (true ? (String.format("%.3f V", floatValue)) : ""));
